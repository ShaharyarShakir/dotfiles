...
install_arch_dependencies() {
  ...
  DEPENDENCIES='bash zsh bash-completion tar bat tree multitail fastfetch wget unzip fontconfig trash-cli make tmux starship fzf zoxide eza neovim figlet lolcat fd ripgrep yazi stow docker distrobox lazygit btop direnv ttf-jetbrains-mono-git visual-studio-code-bin warp-terminal-bin cursor-bin brave-origin-nightly-bin'
...
link_config() {
  ...
  if [ -e "$OLD_BASHRC" ]; then
    ...
    mv "$OLD_BASHRC" "$OLD_BASHRC.bak" || {
      print_colored "$RED" "Can't move .bashrc!"
      exit 1
    }
  fi
